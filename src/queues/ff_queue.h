...
// - close to no changes from original file, except:
//   - inherit from Queue<u64> interface
//   - added enqueue and dequeue methods to conform with interface
...
class ff_queue final : public Queue<u64>
{
public:
...
private:
  ...
  public:
    bool enqueue(const u64 item) override {
        u64* slot = reinterpret_cast<u64*>(enqueue_prepare(sizeof(u64)));
    ...
    std::optional<u64> dequeue() override {
        const u64* slot = reinterpret_cast<u64*>(dequeue_prepare());
